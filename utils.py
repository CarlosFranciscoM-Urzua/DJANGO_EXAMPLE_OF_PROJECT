...
def backup_database():
    ...
    backup_filename = f"backup_{db_name}_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.sql"
    ...
    try:
        ...
        command = f"mysqldump -u {db_user} -p {db_name} > {backup_path}"
        subprocess.run(command, shell=True, check=True)
        #print (f"{db_password}")
        #subprocess.run("\n", shell=True, check=True)
        #subprocess.run(db_password, shell=True, check=True)
        return backup_path
...
def restore_database(sql_file):
    ...
    try:
        ...
        #command = f"mysql -u {db_user} -p {db_name} < {sql_file}"
        subprocess.run(command, shell=True, check=True)
        return True
    except subprocess.CalledProcessError as e:
